...
router.get(
  "/users/notAccept", [AuthenticationMiddleware, AuthuthrationMiddleware],
  async (req: Request, res: Response) => {
    const users: any = await User.find({ isAdmin: false, activateBouqute: false })
    res.send(users);
  }
);
router.get(
  "/users/accept", [AuthenticationMiddleware, AuthuthrationMiddleware],
  async (req: Request, res: Response) => {
    const users: any = await User.find({ isAdmin: false, activateBouqute: true })
    res.send(users);
  }
);
router.get(
  "/users/accept/:id", [AuthenticationMiddleware, AuthuthrationMiddleware],
  async (req: Request, res: Response) => {
...
router.get(
  "/admin/user/:id", [AuthenticationMiddleware, AuthuthrationMiddleware],
  async (req: Request, res: Response) => {
    const user: any = await User.find({_id:req.params.id })
    res.send(user[0]);
  }
);
